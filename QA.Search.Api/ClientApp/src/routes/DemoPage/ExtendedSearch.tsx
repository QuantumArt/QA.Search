...
const ExtendedSearch = withRouter(({ location, history, delayMsec = 100 }: Props) => {
  ...
  useDebounce({
    async action(signal) {
      // TODO: uncomment when propper completion will be implemented
      if (true) return;

      if (!queryString) return;
      try {
        const resp = await new SearchClient().completion<SearchItem>(signal, {
          $from: selectedIndex || "*",
          $query: queryString,
          $weights: {
            Description: 1,
          ...
          $limit: COMPLETION_ROWS
        });

        setCompletionPhrases(resp.phrases);
      } catch (err) {
  ...
  const extendedSearch = useCallback(
    async ({ index, query, offset }) => {
      history.replace(`/demo?${qs.stringify({ query })}`);
      if (abortRef.current) {
      ...
      try {
        const resp = await new SearchClient().searchObsolete<SearchItem>(abortRef.current.signal, {
          $from: index || "*",
          $select: [
            "SearchUrl",
            "Title",
